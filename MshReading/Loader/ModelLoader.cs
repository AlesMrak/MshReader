...
namespace proj
{
    class ModelLoader
    {
        ...
        class FileContext
        {
            ...
            public bool ReadChunk()
            {
                ...
                if (!IsEOF())
                {
                    ...
                    // print out the name and depth
#if MAXLOADER_DEBUG
                    ModelLoader.WriteDebug("FileContext::ReadChunk(): SP: [{0:d02}] ID: 0x{1:x04}: OFF: {2:d5}, LEN: {3:d5}",
                            sp,
                            chunk.id,
                            chunk.off,
                            chunk.len);
            ...
            {
#if MAXLOADER_DEBUG
                ModelLoader.WriteDebug("FileContext::SkipChunk(): SP: [{0:d02}], ID: 0x{1:x04}, OFF: {2:d5}, LEN: {3:d5}",
            ...
            void _PushChunk()
            {
                ...
                stack[sp++] = chunk;
#if MAXLOADER_DEBUG
                ModelLoader.WriteDebug("FileContext::_PushChunk(): SP: [{0:d02}], ID: 0x{1:x04}, OFF: {2:d5}, LEN: {3:d5}",
                    sp, chunk.id, chunk.off, chunk.len);
#endif
            }
        ...
        class FaceMaterial : IChunkHandler
        {
            public string name;
            public ushort ne;
            public short[] faces;
        ...
        class MeshObject : IChunkHandler
        {
            public string name;
            public ushort mnv;
            public float[] mc;
        ...
        class CameraObject : IChunkHandler
        {
            public string name;
            public float[] pos;
            public float[] target;
            ...
            public bool ProcessChunk(FileContext ctx)
            {
                ...
                pos[1] = ctx.PopFloat();
                pos[2] = ctx.PopFloat();
                target[0] = ctx.PopFloat();
                target[1] = ctx.PopFloat();
                target[2] = ctx.PopFloat();
                bank = ctx.PopFloat();
                lens = ctx.PopFloat();

                // skip any unused or undocumented camera chunks
                while (ctx.ReadChunk())
                {
                    ctx.SkipChunk();
                }
        ...
        class LightObject : IChunkHandler
        {
            string name;
            float[] pos;
            Color3f color3f;
            ...
            public LightObject(string objname)
            {
                name = objname;
                pos = new float[3];
                color3f = new Color3f();
            ...
            public bool ProcessChunk(FileContext ctx)
            {
                ...
                while (ctx.ReadChunk())
                {
                    ...
                    {
                        case kColor24:
                            color24.ProcessChunk(ctx);
                            break;
                        case kColor3f:
                            color3f.ProcessChunk(ctx);
                            break;
                        case kLightSpot:
                            isSpot = true;
                            target[0] = ctx.PopFloat();
                            target[1] = ctx.PopFloat();
                            target[2] = ctx.PopFloat();
                            hotspot = ctx.PopFloat();
                            falloff = ctx.PopFloat();
                            break;
                        default:
                            ctx.SkipChunk();
                            break;
        ...
        List<LightObject> _lights;
        List<CameraObject> _cameras;
        ...
        public ModelLoader()
        {
            ...
            _cameras = new List<CameraObject>();
            ...
            while (ctx.ReadChunk())
            {
                switch (ctx.chunk.id)
                {
                    case kNamedObject:      // (object block
                        name = ctx.PopString();
#if MAXLOADER_DEBUG
                        ModelLoader.WriteDebug("Loader::ReadObjects(): named object: {0}", name);
#endif
                        break;
                    case kObjMesh:
                        {
                    ...
                    case kObjLight:
                        {
                            LightObject light = new LightObject(name);
                            if (light.ProcessChunk(ctx))
                            {
                                _lights.Add(light);
                            }
                        }
                    ...
                    case kObjCamera:
                        {
                            CameraObject cam = new CameraObject(name);
                            if (cam.ProcessChunk(ctx))
                            {
                                _cameras.Add(cam);
                            }
                        }
        ...
        {
#if MAXLOADER_DEBUG
            ModelLoader.WriteDebug("Loader::ReadKeyFramedata(): position: {0}", ctx.Position());
#endif
            while (ctx.ReadChunk())
            {
                switch (ctx.chunk.id)
                    ...
                    case kKeyFrameHdr:
                        {
                            ...
                            ModelLoader.WriteDebug("KeyFrame Header: rev: 0x{0:x04}, animlen: 0x{1:x08}, cstr: {2}", revision, animlen, cstr);
                        }
                        break;
                    case kKeyFrameSegment:
                        {
                            ...
                            ModelLoader.WriteDebug("KeyFrame Segment: start: 0x{0:x08}, end: 0x{1:x08}", start, end);
                        }
                        break;
                    case kKeyFrameCurtime:
                        {
                            uint curtime = ctx.PopDword();
                            ModelLoader.WriteDebug("KeyFrame curtime: 0x{0:x08}", curtime);
                        }
                        break;
                    case kObjectNodeTag:
                        ReadObjectNode(ctx);
                    ...
                    case kKeyFrameNodeID:
                        {
                            ...
                            ModelLoader.WriteDebug("KeyFrame ID: 0x{0:x04}", nodeID);
                        }
                        break;
                    case kKeyFrameNodeHdr:
                        {
                            ...
                            ModelLoader.WriteDebug("KeyFrame NodeHdr: {0}, flags1: 0x{1:x04}, flags2: 0x{2:x04}, heirarchy: 0x{3:x04}", name, flags1, flags2, heirarchy);
                        }
                        break;
                    case kKeyFramePos:
                        {
                            ...
                            ModelLoader.WriteDebug("KeyFrame POS: flags: 0x{0:x04}, keys: 0x{1:x04}, frame: 0x{2:x04}", flags, keys, framenum);
                            ModelLoader.WriteDebug("KeyFrame POS: x: {0:f2}, y: {1:f2}, z: {2:f2}", x, y, z);
                            ModelLoader.WriteDebug("KeyFrame POS: unk[{0},{1},{2},{3}], unk2: {4}, unk3: {5}", unk[0], unk[1], unk[2], unk[3], unk2, unk3);
                    ...
                    case kKeyFramePivot:
                        {
                            ...
                            float y = ctx.PopFloat();
                            float z = ctx.PopFloat();
                            ModelLoader.WriteDebug("KeyFrame Pivot: x: {0:f2}, y: {1:f2}, z: {2:f2}", x, y, z);
                    ...
                    case kKeyFrameRot:
                        {
                            ...
                            ModelLoader.WriteDebug("KeyFrame ROT: flags: 0x{0:x04}, keys: 0x{1:x04}, frame: 0x{2:x04}", flags, keys, framenum);
                            ModelLoader.WriteDebug("KeyFrame ROT: x: {0:f2}, y: {1:f2}, z: {2:f2}, rad: {3:f2}", x, y, z, r);
                            ModelLoader.WriteDebug("KeyFrame ROT: unk[{0},{1},{2},{3}], unk2: {4}, unk3: {5}", unk[0], unk[1], unk[2], unk[3], unk2, unk3);
                    ...
                    case kKeyFrameScale:
                        {
                            ...
                            ModelLoader.WriteDebug("KeyFrame SCL: flags: 0x{0:x04}, keys: 0x{1:x04}, frame: 0x{2:x04}", flags, keys, framenum);
                            ModelLoader.WriteDebug("KeyFrame SCL: x: {0:f2}, y: {1:f2}, z: {2:f2}", x, y, z);
                            ModelLoader.WriteDebug("KeyFrame SCL: unk[{0},{1},{2},{3}], unk2: {4}, unk3: {5}", unk[0], unk[1], unk[2], unk[3], unk2, unk3);
        ...
        public void LoadMesh(string name, Mesh mesh, float scale)
        {
        ...
        public void LoadAllMeshes(List<Mesh> outMeshes, float scale)
        {
            uint i;
            ...
            {
                Mesh mesh = new Mesh();
                LoadMesh(i, mesh, scale, false);
                outMeshes.Add(mesh);
            }
        }

        public void LoadMesh(uint i, Mesh mesh, float scale, bool applySmoothingGroups)
        {
            Debug.Assert(i < _meshes.Count);

            MeshObject meshobj = _meshes[(int)i];

            mesh.Allocate(meshobj.nVerts, meshobj.nFaces);

            // scale and transform the mesh object
            for (i = 0; i < meshobj.nVerts; ++i)
            {
                Vector3 v = new Vector3(meshobj.verts[i].x, meshobj.verts[i].y, meshobj.verts[i].z);
                mesh.SetVertexPosition(i, v);
            }

            // Buffers for calculating smoothing groups
            ushort[] sharedFaces = new ushort[mesh.verts.Length];
            Vector3[] summedNormals = new Vector3[mesh.verts.Length];
            Vector3 v1, v2, normal;

            for (i = 0; i < meshobj.nFaces; ++i)
            {
                Face f = meshobj.faces[i];

                // copy face information
                mesh.SetFaceIndicies(i, meshobj.faces[i].a, meshobj.faces[i].b, meshobj.faces[i].c);
                ...
                mesh.SetVertexNormal(f.a, normal);
                mesh.SetVertexNormal(f.b, normal);
                mesh.SetVertexNormal(f.c, normal);
            ...
            if (applySmoothingGroups)
            {
                ...
                for (int iv = 0; iv < mesh.verts.Length; ++iv)
                {
